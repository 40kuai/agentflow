...
- [ ] Task 7: `owns` 路径级强制（越界检出）
  - [ ] SubTask 7.1: 节点结束后采集其工作区实际变更路径（`git status`/`git diff --name-only` 等价手段）
  - [ ] SubTask 7.2: 与 `owns` 匹配（含通配），越界即记违规事件并附越界路径清单
  - [ ] SubTask 7.3: 明确越界后的任务语义（不得静默继续），并在事件与失败原因中体现
  - [ ] SubTask 7.4: 补测试覆盖"越界检出"与"未越界不误报"两个方向

- [ ] Task 8: 并行前路径占用检查（把冲突挡在启动之前）
  - [ ] SubTask 8.1: 实现 `owns` 重叠检测（含通配前缀重叠判定）
  - [ ] SubTask 8.2: 在同一批次启动**之前**执行检查；重叠时按配置改为串行或拒绝，并给出明确原因（涉及哪两个节点、哪段路径）
  - [ ] SubTask 8.3: 补测试：重叠被挡、不相交被允许（两侧都要有断言）

- [ ] Task 9: 拓扑支持 fan-out 与 join
  - [ ] SubTask 9.1: 让决策支持**多出边同时激活**（一批次多个就绪节点）
  - [ ] SubTask 9.2: 支持 join 语义：节点等待**全部**上游进入终态后才启动
  - [ ] SubTask 9.3: 补测试：扇出后多节点就绪、join 未满足时不启动、join 满足后启动

- [ ] Task 10: 并发执行与并发上限
  - [ ] SubTask 10.1: 主循环改为可同时推进多个就绪节点
  - [ ] SubTask 10.2: 消费 `AGENTFLOW_GLOBAL_CONCURRENCY`（当前无消费者），实际并发数不超过上限，其余排队
  - [ ] SubTask 10.3: 保证无并发时行为与既有串行**等价**（现有单测全绿）
  - [ ] SubTask 10.4: 补测试：并发上限被遵守、超限节点排队
