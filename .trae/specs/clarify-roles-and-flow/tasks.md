...
- [x] **跨任务冲突（中）→ 已修复（`b490be4`）**：本 spec 的 Task 12 新增了 3 个角色（为让并行节点 `owns` 互不相交），曾使用户的 `tests/website/website.test.ts` 的 AC5「角色数=3」失败。已按建议把该断言改为**包含式**（断言 `simple_dev` 用到的 `pm`/`backend_dev`/`qa_engineer` 都在 `config/roles` 中注册，不写死总数），而非把 3 改成 6。
- [x] **未跟踪的 `README.md` → 已订正（`b490be4`）**：核实其性质为**关于 AgentFlow 平台本身的说明**（非无关产物），已如实订正 6 处过期表述。因该文件未跟踪，改动留在工作区、未纳入提交。
- [x] **`web/` 内的过期文案 → 已清理（`b490be4`）**：`web/src/App.tsx`、`web/src/components/NodeCostView.tsx` 的「无消费者」等过期字符串已改（**仅改文案，未动任何前端逻辑**）。
- [ ] **`on_missing` 仅为声明式（信息，仍未发生）**：边的失败语义字段已可读，但内核尚未消费它（join 等待语义由主循环保证）。如需内核依它区分"失败 vs 等待"，应另开任务。
- [x] **`parallel_dev` 未接线 → 已接线（`b490be4`）**：新增 `AGENTFLOW_WORKFLOW`（默认 `simple_dev`，默认行为不变），可通过该变量选用 `parallel_dev`。**注**：该键**未加入 `.env.example`**——因用户的官网测试断言「官网环境变量表与 `.env.example` 键名/默认值完全一致」，加键会破该断言；变量仍可经 `.env`/环境变量生效。
- [x] **端到端冒烟**：已执行并 **PASS**（见 SubTask 14.5）。过程中暴露并修掉了两个此前单测覆盖不到的缺陷：
  - [x] **产品级缺陷（高，已修 `47cc2d0`）**：`owns` 越界核对把**平台自己的运行时文件**（`logDir` 的 `logs/runs/*.jsonl`、事件库、workspace 根）当成节点改动 → 默认配置下只读角色（`owns: []`）**必被误判失败**，即安全闸在默认配置下让整个平台不可用。修法：核对前排除**配置注入的**运行时路径（非硬编码字面量），并用 `realpath` 归一消除 macOS `/tmp→/private/tmp` 符号链接盲区。真实越界仍被检出（双向测试 + 变异验证）。
  - [x] **冒烟脚本缺陷（中，已修 `2c543aa`）**：目标端口被占用时脚本会**静默地对错误进程做断言**（白跑一次并浪费额度）。修法：未显式指定端口时自动选用 8790–8899 空闲端口，显式指定且被占用则明确中文报错退出；**不 kill 他人进程**。
  - [ ] **遗留环境问题（需你处置）**：8787 端口上有**一个遗留的开发服务**在运行（冒烟期间确认存在，未被 kill）。若它是你不再使用的旧进程，建议自行清理。